  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-sm text-[var(--muted)] mt-1">Verwalten Sie Ihr Profil, Sprache und Datenschutz-Einstellungen</p>
      </div>
      ...
      <div className="max-w-2xl space-y-5">
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="mt-3 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2 text-xs text-amber-800">
            Ausgabe-Sprachen außer Deutsch sind mit dem Hinweis &ldquo;Nur zur Überprüfung&rdquo; versehen, da deutsche Behörden deutsche Schreiben erfordern.
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          <div className="flex items-start justify-between mb-4">
            <h2 className="font-semibold text-sm text-[var(--foreground)]">Benachrichtigungen</h2>
            <AccountClient section="notifications" />
          </div>
          <div className="space-y-3">
            {[
              { label: 'Frist-Erinnerungen (7 Tage vorher)', enabled: true },
              { label: 'Frist-Erinnerungen (1 Tag vorher)', enabled: true },
              { label: 'KI-Pipeline abgeschlossen', enabled: true },
              { label: 'Marketing & Neuigkeiten', enabled: false },
            ].map((pref) => (
              <div key={pref.label} className="flex items-center justify-between py-2 border-b border-[var(--border)] last:border-0">
                <span className="text-sm text-[var(--foreground)]">{pref.label}</span>
                <NotificationToggle enabled={pref.enabled} />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-[var(--border)] p-5">
          ...
          <div className="flex gap-3 flex-wrap">
            <button
              ...
              className="text-sm border border-[var(--border)] px-3 py-1.5 rounded-lg hover:bg-gray-50 transition-colors"
            >
              Daten exportieren
            </button>
            <button
...
function NotificationToggle({ enabled }: { enabled: boolean }) {
  return (
    <div
      className={`w-9 h-5 rounded-full cursor-pointer relative transition-colors ${enabled ? 'bg-brand-600' : 'bg-gray-200'}`}
      onClick={() => alert('Benachrichtigungseinstellungen werden in einer zukünftigen Version bearbeitbar sein.')}
    >
      <div
        className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow-sm transition-transform ${enabled ? 'translate-x-4' : 'translate-x-0.5'}`}
      />
    </div>
  )
}
